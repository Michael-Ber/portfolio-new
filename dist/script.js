...
const form = formSelector => {
  ...
  form.addEventListener('submit', function (e) {
    ...
    Object(_services_sendRequest__WEBPACK_IMPORTED_MODULE_0__["sendReq"])("https://portfolio-mailer-8yue.onrender.com/send_mail", jsonData).then(res => {
      ...
      showModal({
        error: err
      }, window.location.hash.substring(1), modal);
    }).finally(() => {
      this.reset();
    ...
    function showModal(condition, hash, modalElem) {
      ...
      if (condition.hasOwnProperty('success')) {
        ...
        modal.innerHTML = `   
                        ...
                        <div class="modal__msg">${message['sending-error'][hash]}</div>
                        <div class="modal__close" data-close>&#10005;</div>
                    `;
      }
    }
...
const sectionCommutator = () => {
  ...
  function triggerSlider(e) {
    ...
    if (document.querySelector('.burger__menu').classList.contains('burger__menu_active')) {
      Object(_burger__WEBPACK_IMPORTED_MODULE_0__["closeBurgerMenu"])(document.querySelector('.burger__btn'), document.querySelector('.burger__menu'), 'burger__btn_active', 'burger__menu_active', document.body);
    }
  }
};
